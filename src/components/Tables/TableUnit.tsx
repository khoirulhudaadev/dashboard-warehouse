...
const TableUnit: React.FC<DynamicTableProps> = ({columns, data, title, onDelete, onUpdate}) => {

  const [search, setSearch] = useState<string>('')

  const filteredData = search !== '' 
  ? data?.filter((d: any) => d.unit_name.toLowerCase().includes(search.toLowerCase())) 
  : data;
  ...
  const generatePDF = ({ title, data }: PDFInterface) => {
    ...
    doc.autoTable({
      startY: y,
      head: [['No', 'ID-satuan', 'Nama satuan', 'Tgl dibuat']],
      body: mappedData,
    });
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-10">
      <div className='flex items-center justify-between mb-4'>
        ...
        <div className='flex w-max'>
          <Button text='Tambah satuan' url='/tambah-satuan' icon={<Add01Icon />} className='bg-blue-600' />
          <Button text='Unduh PDF' onClick={() => generatePDF({title: 'Data satuan barang', data})} icon={<Pdf01Icon />} className='bg-red-600' />
        </div>
      </div>
          ...
          {filteredData && filteredData?.length > 0 ? 
            filteredData?.map((d:any, index: number) => (
              <Table.Row key={index} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                ...
                <Table.Cell className='w-[40%]'>{d?.unit_name}</Table.Cell>
                <Table.Cell>{d?.unit_id}</Table.Cell>
                <Table.Cell>{formatDate(d?.created_at)}</Table.Cell>
                <Table.Cell className='flex items-center gap-4'>
                  <Delete01Icon onClick={() => onDelete?.(d.unit_id ?? 0)} className='text-red-500' />
                  <PencilEdit01Icon onClick={() => onUpdate?.(d?.unit_id, d)} className='text-yellow-500' />              
                </Table.Cell>
              </Table.Row>
            )):
              null
